...
こんにちは、hinoshiba です。  
今回、宅内ドキュメントをインターネット上に**公開**する対応をとったので、経緯などをブログにまとめておこうと思い、記事を作成しています。  
...
家電量販店で、物品を購入するときに、宅内ドキュメントをさっと確認し型番を確認することができたりと、まとめておく癖がついていると、ふとしたときに恩恵を受けられます。  
...
例えば、家庭で利用しているWindowsのプロダクトキーなどは、流石に漏れてほしくないものなので、オンプレの完全非公開 で運用しています。  
オンプレでは、[Gitea](https://gitea.io/en-us/) を採用しており、クラウドは、みなさんもご存知 [GitHub](https://github.com/) を採用しています。  
...
`普通`という言葉は、「広く通用する状態([Wikipedia: 普通 より引用](https://ja.wikipedia.org/wiki/%E6%99%AE%E9%80%9A)」 という意味を持つ危険な言葉です。
というのも`普通`について、どうせみんな知っているので、わざわざ説明する必要がないと判断されがちかと思います。  
しかし、`広く通用する`は、TPOによって異なるので、状況に応じて説明が必要なケースもありえます。  
説明すべきときにも説明されない`普通`とても危険ですよね。  
今回発生した問題も、東京比、田舎である、私の実家などでは`普通`で受け入れられそうな事象でもありました。  

さて、いくら特定の地域や家庭によって`普通`だとしても、本件によって妻へ迷惑をかけてしまった事実があり、再発防止策を検討する必要があります。  
...
この手順ですが、非公開にするような情報が含まれておらず、他に場所がないから一色帯に宅内ドキュメントにまとめているだけでした。  
...
例えば、弊家には、トイレにスリッパがありません。状況をしらなければ不衛生と感じてしまうと思いますが、そこからわざわざスリッパがないことを会話するともあまり思えません。なので、なぜ無いのか書いておくと少しストレス軽減されるのでは無いかと思っています。  

---

さて、これにて、弊家の宅内ドキュメントは、機密度に応じた3段階のドキュメント管理が行われ始めました。  
これからもPDCAを回していけばJIS Q 27001 準拠な誤家庭の誕生も近いかもしれませんね。  
という冗談は置いておいて、そこまでルールをガチガチにしないにしても、公開する事で双方が幸せになりそうな仕組みは引き続き検討し、公開していこうと思います。  
